...
XMPP__PT_PROVIDER::XMPP__PT_PROVIDER(const char *par_port_name)
	: PORT(par_port_name),
#ifdef AS_USE_SSL
...
XMPP__PT_PROVIDER::~XMPP__PT_PROVIDER()
{
	delete header_descr;
}
...
{
  log_debug("entering XMPP__PT_PROVIDER::set_parameter(%s, %s)", parameter_name,
    parameter_value);
  if(strcmp(parameter_name, "packet_hdr_length_offset") == 0) {
    ...
    if(!parameter_set(parameter_name ,parameter_value))
      TTCN_warning("XMPP__PT_PROVIDER::set_parameter(): Unsupported Test Port parameter: %s", parameter_name);
  }
  log_debug("leaving XMPP__PT_PROVIDER::set_parameter(%s, %s)", parameter_name, parameter_value);
}

void XMPP__PT_PROVIDER::Handle_Fd_Event(int fd,
  boolean is_readable, boolean is_writable, boolean is_error)
{
  log_debug("entering XMPP__PT_PROVIDER::Handle_Fd_Event()");
  Handle_Socket_Event(fd, is_readable, is_writable, is_error);
  log_debug("leaving XMPP__PT_PROVIDER::Handle_Fd_Event()");
}

void XMPP__PT_PROVIDER::Handle_Timeout(double time_since_last_call)
{
...
void XMPP__PT_PROVIDER::peer_disconnected(int client_id) {
  log_debug("entering XMPP__PT_PROVIDER::peer_disconnected()");
  if(get_use_connection_ASPs())
  {
    XMPP__Types::XMPP__Close asp;
    asp.client__id() = client_id;
    incoming_message(asp);
  }
  else Abstract_Socket::peer_disconnected(client_id);
  log_debug("leaving XMPP__PT_PROVIDER::peer_disconnected()");
}

void XMPP__PT_PROVIDER::user_map(const char */*system_port*/)
{
  log_debug("entering XMPP__PT_PROVIDER::user_map()");
  ...
  if(!get_use_connection_ASPs())
    map_user();
  log_debug("leaving XMPP__PT_PROVIDER::user_map()");
}

void XMPP__PT_PROVIDER::user_unmap(const char */*system_port*/)
{
  log_debug("entering XMPP__PT_PROVIDER::user_unmap()");
  unmap_user();
  delete header_descr;
  header_descr = NULL;
  log_debug("leaving XMPP__PT_PROVIDER::user_unmap()");
}

void XMPP__PT_PROVIDER::user_start()
{
  log_debug("entering XMPP__PT_PROVIDER::user_start()");
  log_debug("leaving XMPP__PT_PROVIDER::user_start()");
}

void XMPP__PT_PROVIDER::user_stop()
{
  log_debug("entering XMPP__PT_PROVIDER::user_stop()");
  log_debug("leaving XMPP__PT_PROVIDER::user_stop()");
}

void XMPP__PT_PROVIDER::outgoing_send(const XMPP__Types::XMPP__Connect& send_par)
{
  log_debug("entering XMPP__PT_PROVIDER::outgoing_send(XMPP__Connect)");
  ...
  log_debug("leaving XMPP__PT_PROVIDER::outgoing_send(XMPP__Connect)");
}

void XMPP__PT_PROVIDER::report_error(int client_id, int msg_length, int sent_length, const unsigned char* msg, const char* error_text)
{
  /*log_debug("entering XMPP__PT_PROVIDER::report_error");
  if(get_use_connection_ASPs()){
    XMPP__Types::XMPP__Send__error asp;
  ...
  log_debug("leaving XMPP__PT_PROVIDER::report_error");*/
}

const PacketHeaderDescr *XMPP__PT_PROVIDER::Get_Header_Descriptor() const
{
  return header_descr;
}

void XMPP__PT_PROVIDER::client_connection_opened(int client_id)
{
  log_debug("entering XMPP__PT_PROVIDER::client_connection_opened(%d)", client_id);

  if(get_use_connection_ASPs())
  {
    XMPP__Types::XMPP__Connect__result asp;
  ...
  log_debug("leaving XMPP__PT_PROVIDER::client_connection_opened()");
}

void XMPP__PT_PROVIDER::peer_connected(int client_id, const char * host, const int port)
{
  log_debug("entering XMPP__PT_PROVIDER::peer_connected(%d, %s, %d)", client_id, host, port);

  if(get_use_connection_ASPs())
  {
    XMPP__Types::XMPP__Connected asp;
  ...
  log_debug("leaving XMPP__PT_PROVIDER::peer_connected()");
}

void XMPP__PT_PROVIDER::outgoing_send(const XMPP__Types::XMPP__Close& send_par)
{
  log_debug("entering XMPP__PT_PROVIDER::outgoing_send(XMPP__Close)");
  ...
  log_debug("leaving XMPP__PT_PROVIDER::outgoing_send(XMPP__Close)");
}

void XMPP__PT_PROVIDER::outgoing_send(const XMPP__Types::XMPP__Listen& send_par)
{
  log_debug("entering XMPP__PT_PROVIDER::outgoing_send(XMPP__Listen)");
  ...
  log_debug("leaving XMPP__PT_PROVIDER::outgoing_send(XMPP__Listen)");
}

void XMPP__PT_PROVIDER::listen_port_opened(int port_number)
{
  log_debug("entering XMPP__PT_PROVIDER::listen_port_opened(%d)", port_number);

  if(get_use_connection_ASPs())
  {
    XMPP__Types::XMPP__Listen__result asp;
    asp.portnumber() = port_number;
    incoming_message(asp);
  }
  else Abstract_Socket::listen_port_opened(port_number);

  log_debug("leaving XMPP__PT_PROVIDER::listen_port_opened()");
}

void XMPP__PT_PROVIDER::outgoing_send(const XMPP__Types::XMPP__Shutdown& /*send_par*/)
{
  log_debug("entering XMPP__PT_PROVIDER::outgoing_send(XMPP__Shutdown)");

  close_listen_port();

  log_debug("leaving XMPP__PT_PROVIDER::outgoing_send(XMPP__Shutdown)");
}

void XMPP__PT_PROVIDER::outgoing_send(const XMPP__Types::XMPP__SetAddressFamily& send_par)
{
  log_debug("entering XMPP__PT_PROVIDER::outgoing_send(XMPP__SetAddressFamily)");

  if (send_par == XMPP__Types::c__TCP__AF__INET || send_par == XMPP__Types::c__TCP__IPv4) {
    set_ai_family(AF_INET);
    log_debug("Address family set to %s", (const char*)send_par);
  } else if (send_par == XMPP__Types::c__TCP__AF__INET6 || send_par == XMPP__Types::c__TCP__IPv6) {
    set_ai_family(AF_INET6);
    log_debug("Address family set to %s", (const char*)send_par);
  } else if(send_par == XMPP__Types::c__TCP__AF__UNSPEC || send_par == XMPP__Types::c__TCP__UNSPEC) {
  ...
  log_debug("leaving XMPP__PT_PROVIDER::outgoing_send(XMPP__SetAddressFamily)");
}

const char* XMPP__PT_PROVIDER::local_port_name()              { return "serverPort";}
const char* XMPP__PT_PROVIDER::remote_address_name()          { return "destIPAddr";}
const char* XMPP__PT_PROVIDER::local_address_name()           { return "serverIPAddr";}
const char* XMPP__PT_PROVIDER::remote_port_name()             { return "destPort";}
const char* XMPP__PT_PROVIDER::ai_family_name()               { return "addressFamily";}
const char* XMPP__PT_PROVIDER::halt_on_connection_reset_name(){ return "halt_on_connection_reset";}
const char* XMPP__PT_PROVIDER::server_mode_name()             { return "server_mode";}
const char* XMPP__PT_PROVIDER::socket_debugging_name()        { return "socket_debugging";}
const char* XMPP__PT_PROVIDER::nagling_name()                 { return "nagling";}
const char* XMPP__PT_PROVIDER::server_backlog_name()          { return "server_backlog";}
const char* XMPP__PT_PROVIDER::ssl_use_ssl_name()                { return "ssl_use_ssl";}
const char* XMPP__PT_PROVIDER::ssl_use_session_resumption_name() { return "ssl_use_session_resumption";}
const char* XMPP__PT_PROVIDER::ssl_private_key_file_name()       { return "ssl_private_key_file";}
const char* XMPP__PT_PROVIDER::ssl_trustedCAlist_file_name()     { return "ssl_trustedCAlist_file";}
const char* XMPP__PT_PROVIDER::ssl_certificate_file_name()       { return "ssl_certificate_chain_file";}
const char* XMPP__PT_PROVIDER::ssl_password_name()               { return "ssl_private_key_password";}
const char* XMPP__PT_PROVIDER::ssl_cipher_list_name()            { return "ssl_allowed_ciphers_list";}
const char* XMPP__PT_PROVIDER::ssl_verifycertificate_name()      { return "ssl_verify_certificate";}
